...
class LazyListTest extends TestCase {
	...
	public function testLift(): void {
		$lifted = LazyList::lift(function ($a, $b) { return $a . $b; });
	...
	public function testReduce(): void {
		...
		$sum = $lazyList->reduce(function ($sum, $item) { return $sum + $item; }, 0);
	...
	public function testScan(): void {
		...
		$sum = $lazyList->scan(function ($sum, $item) { return $sum + $item; }, 0);
	...
	public function testTakeWhile(): void {
		$lazyList = new LazyList(new ArrayIterator(range(1, 10)));
		$taken = $lazyList->takeWhile(function (int $i): bool { return $i <= 3; });
	...
	public function testChunk(): void {
		...
		$this->assertEquals(
			...
			$chunked->map(function ($chunk) { return iterator_to_array($chunk); })->toArray()
		);
	}
	...
	public function testDropWhile(): void {
		...
		$rest = $lazyList->dropWhile(function(int $item) { return $item < 5; });
	...
	public function testExists(): void {
		...
		$this->assertTrue($lazyList->exists(function(int $item) { return $item > 5; }));
		$this->assertFalse($lazyList->exists(function(int $item) { return $item > 10; }));
	}

	public function testAll(): void {
		$lazyList = new LazyList(new ArrayIterator(range(2, 10, 2)));

		$this->assertTrue($lazyList->all(function(int $item) { return $item < 11; }));
		$this->assertTrue($lazyList->all(function(int $item) { return $item % 2 == 0; }));
		$this->assertFalse($lazyList->all(function(int $item) { return $item < 10; }));
	}
