...
phpstan:
	docker run -it --rm -v ${PWD}:/app -w /app php:7.3-cli-alpine php -d memory_limit=-1 bin/phpstan --ansi analyse

phpunit:
	docker run -it --rm -v ${PWD}:/app -w /app php:7.3-cli-alpine php bin/phpunit --colors=always
